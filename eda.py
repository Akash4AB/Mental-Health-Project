    ...
    if submenu == "Visualization":
        ...
        with st.expander("Supporting Facilities"):
            ...
            for i, variable in enumerate(supporting_facilities, 1):
                plt.subplot(2, 3, i)
                if i <= 3:
                    sns.countplot(data=filtered_data, x=variable, order=filtered_data[variable].value_counts().index,
                                  color='#ec838a')
                else:
                    sns.countplot(data=filtered_data, x=variable, order=filtered_data[variable].value_counts().index,
                                  color='#9b9c9a')
        ...
        with st.expander("Transperency and Confidentiatlity"):
            ...
            transperency_and_confidentiality = ['anonymity', 'mental_health_consequence', 'coworkers', 'supervisor',
                                                'obs_consequence']
        ...
        with col1:
            with st.expander("Country Distribution"):
                st.dataframe(
                    df_clean.groupby(by='Country').agg(Qty=('Country', 'count')).sort_values('Qty', ascending=False))
        with col2:
            with st.expander("Gender Distribution"):
                st.dataframe(
                    df_clean.groupby(by='Gender').agg(Qty=('Gender', 'count')).sort_values(by='Qty', ascending=False))
        with col3:
            with st.expander("Age Distribution"):
                # age restriction
                df_clean_age = df_clean[(df['Age'] >= 17) & (df_clean['Age'] <= 99)]
                st.dataframe(
                    df_clean_age.groupby(by='Age').agg(Qty=('Age', 'count')).sort_values(by='Qty', ascending=False))
