...
from sklearn.metrics import confusion_matrix, accuracy_score, f1_score,precision_score, _dist_metrics, recall_score, fbeta_score
...
def load_model(ml_model):
    if ml_model == 'Logistic Regression':
        return jl.load('LR_Classifier.pkl')
    elif ml_model == 'Decision Tree':
        return jl.load('DT_Classifier.pkl')
    elif ml_model == 'Random Forest Classifier':
        return jl.load('RF_Classifier.pkl')
    elif ml_model == 'KNeighborsClassifier':
        return jl.load('KNN.pkl')
    elif ml_model == 'Support Vector Classifier':
        return jl.load('SVM.pkl')
    elif ml_model == 'XGBoost Classifier':
        return jl.load('XGBoost_Classifier.pkl')
...
def ds_ml_app():
    ...
    with st.expander("Modeling and Evaluation"):
        ...
        st.write(
            "Next, we performed manual encoding for categorical variables with more than two values. The following variables were encoded:")
        ...
        st.write(
            " - 'work_interfere': Replaced 'Never' with 0, 'Rarely' with 1, 'Sometimes' with 2, and 'Often' with 3.")
        st.write(
            " - 'no_employees': Replaced categorical ranges with numerical values, e.g., '1-5' with 0, '6-25' with 1, and so on.")

        st.write("Feature Importance:")
        st.write(
            "To understand the importance of each feature in predicting the need for mental health treatment, we used mutual information (MI) scores. The MI scores provide insights into how strongly each variable influences the 'treatment' column. The results showed that 'work_interfere' and 'family_history' have the highest MI scores, indicating that they are the most influential variables in determining an employee's mental health treatment needs. On the other hand, 'no_employees' and 'tech_company' had MI scores of 0, suggesting that they do not have any predictive power in this context.")

        st.write("Data Splitting:")
        st.write(
            "Before modeling, we split the dataset into dependent and independent variables. The 'treatment' column was designated as the dependent variable, while the rest of the columns were considered independent variables. We used an 80-20 train-test split with stratification to ensure balanced representation of the target variable in both sets.")

        st.write("Feature Scaling:")
        st.write(
            "To standardize the independent variables, we performed feature scaling using the StandardScaler. This step is important for models that are sensitive to the scale of input features, such as logistic regression.")

        st.write("Model Evaluation:")
        st.write(
            "We evaluated the performance of three different models: Logistic Regression, Decision Tree Classifier, and Random Forest. We used two main evaluation metrics - ROC AUC and Accuracy - to assess each model's predictive capabilities.")

        st.write("Logistic Regression:")
        st.write(
            "Despite promising ROC AUC and Accuracy mean scores of 76.75 and 71.72, respectively, we observed issues with this model. When predicting a single sample, the model consistently predicts a treatment value of 1, with probabilities close to 1 for all samples. This behavior indicates that the logistic regression model might be overfitting to the data or encountering convergence issues.")

        st.write("Decision Tree Classifier:")
        st.write(
            "The Decision Tree model showed lower ROC AUC and Accuracy mean scores of 64.98 and 63.29, respectively. Additionally, the probabilities for the predicted classes were consistently between 0 and 1, suggesting that the model may not be effectively capturing the underlying patterns in the data.")

        st.write("Random Forest:")
        st.write(
            "Despite slightly lower ROC AUC mean score of 70.26, the Random Forest model exhibited better generalization. The probabilities of the predicted classes were not highly polarized, indicating a more balanced and reasonable model output.")
    ...
    ml_model = st.selectbox('Machine Learning Model',
    ...
    if prediction[0] == 1:
        st.write(
            "If you or someone you know is struggling with mental health, it's essential to seek help. Reach out to mental health professionals, support groups, or helplines to get the support you need.")
        st.write("- Seeking professional help can make a difference -")
    else:
        st.write(
            "Even if mental health treatment is not predicted to be needed, it's crucial to prioritize your mental well-being. Engaging in self-care practices, talking to friends or family, and seeking professional guidance can contribute to a healthier and happier life.")
